...
input (`Await`) or signaling termination via `Halt`.
  ...
  sealed trait Process[I, O] {
     ...
     */
    def zipWithIndex: Process[I, (O, Int)] = Process.zip(this, count)
  ...
  object Process {
    ...
    def mean: Process[Double, Double] = {
      ...
      def go(n: Int, sum: Double): Process[Double, Double] = Await {
    ...
    def sum2: Process[Double, Double] = loop(0.0)((i, sum) => (i + sum, i + sum))
    ...
    def zip[A, B, C](p1: Process[A, B], p2: Process[A, C]): Process[A, (B, C)] =
      (p1, p2) match {
        ...
        case (Await(recv), _) => Await((a: Option[A]) => zip(recv(a), feed(a)(p2)))
        case (_, Await(recv)) => Await((a: Option[A]) => zip(feed(a)(p1), recv(a)))
      }


    def meanViaZip: Process[Double, Double] = zip(sum, count) |> lift(sc => sc._1 / sc._2)
    ...
    def fileWriter(bw: BufferedWriter): Process[Nothing, Unit] = lift(a => bw.write(a.toString))

    def convertFahrenheitToCelsius(from: java.io.File, to: java.io.File): Unit = {

      val bw = new BufferedWriter(new FileWriter(to))

      val celsiusValues =
        processFile(from, filter[String](s => !s.startsWith("#") && !s.isEmpty)
          |> lift(fahrenheit => toCelsius(fahrenheit.toDouble))
          |> lift(celsius => bw.write(celsius.toString)), ())((_, _) => ())
...
`Process` to a `Sink`.
    ...
    def join[F[_], A](p: Process[F, Process[F, A]]): Process[F, A] = p.flatMap(inner => inner)
