...
object Nonblocking {
  ...
  object Par {
    ...
    def getNth[A](n: Int, l: List[Par[A]]): Par[A] = l match {
      case Nil => throw new IndexOutOfBoundsException
      case h :: t if n == 0 => h
      case h :: t => getNth(n - 1, t)
    }

    def choiceN[A](p: Par[Int])(ps: List[Par[A]]): Par[A] = es => new Future[A] {

      def apply(k: A => Unit): Unit = p(es) { n =>
        eval(es)(getNth(n, ps))
      }
    }
    ...
    def choiceMap[K, V](p: Par[K])(ps: Map[K, Par[V]]): Par[V] = es => new Future[V] {
      ...
      def apply(k: V => Unit): Unit = p(es) { key => eval(es)(ps.get(key)) }
    }
    ...
    def choiceNChooser[A](p: Par[Int])(choices: List[Par[A]]): Par[A] =
      chooser(p)(n => getNth(n, choices))

    def join[A](p: Par[Par[A]]): Par[A] = es => new Future[A] {
      def apply(k: A => Unit): Unit = {
        p(es) { parA => eval(es)(parA(es)(k)) }
      }
    }

    //    join(Par[Par[X]]) == Par[X]
    //    join(Par[Par[X]]) == unit(X)
    ...
    def map2ViaUnitAndFlatmap[A, B, C](a: Par[A], b: Par[B])(f: (A, B) => C): Par[C] =
      flatMap(a)(a => {
        flatMap(b)(b => {
          unit(f(a, b))
        })
