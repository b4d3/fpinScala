...
object IO3 {
  ...
  */
  ...
  def translate[F[_], G[_], A](f: Free[F, A])(fg: F ~> G): Free[G, A] = {
    ...
    val fgToFree = new (F ~> FreeG) {
      override def apply[A](a: F[A]): Free[G, A] = Suspend(fg(a))
    }

    runFree(f)(fgToFree)(freeMonad[G])
  }

  def runConsole[A](a: Free[Console, A]): A = {

    type Id[A] = A
    val t = new (Function0 ~> Id) {
      override def apply[A](f: Function0[A]): Id[A] = f()
    }
    runFree(translate[Console, Function0, A](a)(consoleToFunction0))(t)
  }
    ...
    (cb: Either[Throwable, Array[Byte]] => Unit) => {
      ...
      file.read(buf, fromPosition, (), new CompletionHandler[Integer, Unit] {
        ...
        override def completed(bytesRead: Integer, a: Unit): Unit = {
          ...
          cb(Right(buf.slice().get(arr, 0, bytesRead)))
        }
      })
    }
  }
